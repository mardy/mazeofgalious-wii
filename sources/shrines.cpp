...
void demeter_shrine(unsigned char *screen,int dx,int dy,unsigned char *keyboard)
{
    /* DEMETER - SAVE GAME HERE */
#ifndef GAMEPAD_ONLY
    ...
    if (passage_state==0) {
        ...
        if (keyboard[SWORD_KEY] && !old_keyboard[SWORD_KEY]) {
            ...
            old_keyboard[SWORD_KEY]=true; return;  // prevents 'double button-press' bug
        } /* if */ 
    } /* if */ 
    if (passage_state==1) {
    ...
    if (passage_state==2) {
#ifndef GAMEPAD_ONLY    /* in computer, just show the password */
        char passwd[48];
        char buf[20];
        int k;
...
#else   /* in console, saving is mandatory - present a menu */
        tile_print("CHOOSE THE SLOT WHERE ",TILE_SIZE_X*9,TILE_SIZE_Y*9,screen,dx,dy);
        tile_print("THE CODE WILL BE SAVED",TILE_SIZE_X*9,TILE_SIZE_Y*10,screen,dx,dy);
        tile_print("1  2  3  4  5",TILE_SIZE_X*13,TILE_SIZE_Y*12,screen,dx,dy);        
        if (keyboard[RIGHT_KEY] && !old_keyboard[RIGHT_KEY]) {
            if (currently_selecting<4) {
                currently_selecting++;
                Sound_play(S_select);
            } /* if */ 
        } /* if */ 
        ...
        if (keyboard[SWORD_KEY] && !old_keyboard[SWORD_KEY]) {
            slot = currently_selecting + 1;            
            passage_state=3;
        } /* if */ 
        tiles[152]->
            draw(GAME_VIEW_X+TILE_SIZE_X*(9+currently_selecting*3),GAME_VIEW_Y+12*TILE_SIZE_Y,
                TILE_SIZE_X,TILE_SIZE_Y*2,screen,col_buffer,dx,dy,dx);
#endif 
    } /* if */
    if (slot!=-1) {
...
void passage_mainloop(int map,int map_x,int map_y,unsigned char *screen,int dx,int dy)
{
	...
	if (map==0 && map_x==5 && map_y==9) {
		demeter_shrine(screen, dx, dy, keyboard);
	} /* if */ 
