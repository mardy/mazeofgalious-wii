...
void ReloadGraphics(int dx,int dy)
{
	...
	if (konami_bmp==0 ||
		...
		enemy2_bmp==0 ||
		final_bmp==0) {
		output_debug_message("ERROR: some image buffers cannot be created!\n");
		return;
	} /* if */ 
	...
	if (konami_bmp->get_image()==0 ||
		...
		output_debug_message("ERROR: some graphic files cannot be found!\n");
		return;
	} /* if */ 
	...
	/* An ms personajes de los passages: */ 
	/* DEATH */ 
	...
	/* Todava ms personajes de los passages: */ 
	/* HEPHAESTUS */ 
	...
	/* las flechas no deben chocar con l!															*/ 
