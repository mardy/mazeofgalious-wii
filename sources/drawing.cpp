...
{
	int i;
//    output_debug_message("BEGIN B_rectangle(screen=%p,x=%d,y=%d,dx=%d,dy=%d,pitch=%d,color=%d)",
//                         screen,x,y,dx,dy,pitch,color);    
	...
	for(i=0;i<dy;i++) {
        // TODO crash aqui ao pressionar '+' após ter passado pela fada
        if (y+i >= SCREEN_Y) {
            output_debug_message("[B_rectangle] out of bounds - could crash - breaking and returning imediately\n");
            output_debug_message("(screen=%p,x=%d,y=%d,dx=%d,dy=%d,pitch=%d,color=%d)\n",
                         screen,x,y,dx,dy,pitch,color);
            break;
        } else {
...
//    output_debug_message("END B_rectangle()");
} /* B_rectangle */ 
